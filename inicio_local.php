...
$fecha = date("Y-m-d");
$total_compras = 0.00;
$concepto="CONTEO LOCAL DE VENTA";
$hora=date("H:i:s");
...
$sql_prod=_query("SELECT stock_ubicacion.id_producto, SUM(stock_ubicacion.cantidad) FROM stock_ubicacion JOIN producto ON producto.id_producto=stock_ubicacion.id_producto WHERE id_ubicacion=1  AND  stock_ubicacion.id_estante=0 AND stock_ubicacion.id_posicion=0 GROUP BY stock_ubicacion.id_producto HAVING SUM(stock_ubicacion.cantidad) >0");
...
while ($rows_p=_fetch_array($sql_prod)) {
  ...
  $sql_e_m=_query("SELECT stock_ubicacion.id_su,stock_ubicacion.cantidad FROM stock_ubicacion WHERE stock_ubicacion.id_producto=$id_producto AND stock_ubicacion.id_ubicacion=$destino AND  stock_ubicacion.id_estante=0 AND stock_ubicacion.id_posicion=0 AND cantidad>0");
