...
function cacum()
{
	...
	$id_proveedor = $_POST["id_proveedor"];
	$table = "";
	$sql = _query("SELECT * FROM compra WHERE id_proveedor='$id_proveedor' AND id_sucursal='$id_sucursal' AND CAST(fecha AS DATE) BETWEEN '$fini' AND '$fin' ORDER BY CAST(fecha as DATE) DESC");
	if(_num_rows($sql)>0)
	{
		$entrada = 0;
		$salida = 0;
