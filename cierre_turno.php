...
function initial() {
	...
	$sql_monto_dev=_fetch_array(_query("SELECT SUM(factura.retencion) AS total_retencion FROM factura WHERE id_apertura_pagada=$id_apertura AND credito=0"));
	$monto_retencion=$sql_monto_dev['total_retencion'];


	$total_corte = $total_tike + $total_factura- $monto_dev - $monto_nc  + $total_credito_fiscal + $monto_apertura  + $total_entrada_caja - $total_salida_caja -$monto_retencion;
