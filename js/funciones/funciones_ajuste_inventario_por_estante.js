$(document).ready(function() {
  ...
  $("#producto_buscar2").typeahead({
    ...
    updater: function(selection) {
      ...
      var id_prod = prod[0];
      var descrip = prod[1];
      buscar_producto(id_prod, descrip);
    }
  });
...
function agregar_producto(id_prod, descrip) {
  ...
  var dataString = 'process=consultar_stock' + '&id_producto=' + id_prod+ '&origen=' + origen+ '&estante=' + estante;
  $.ajax({
    type: "POST",
    url: 'ajuste_inventario_por_estante.php',
    data: dataString,
    dataType: 'json',
    success: function(data)
    {
...
function buscar_producto(id_prod, descrip) {
  var hermano="";
  ...
  if (hermano!="") {
    ...
    $('html,body').animate({
  		scrollTop: hermano.find('.cant').offset().top
  	}, 500);
    setTimeout("",500);
    hermano.find('.cant').focus();

  }
  else {
    alert("No se encuentra Agregado");
  }
}
...
$(document).on('change', '#destino', function(event) {
  ...
  $.ajax({
    type: "POST",
    url: "ajuste_inventario_por_estante.php",
    data: "process=estante&id_ubicacion=" + id_ubicacion,
    dataType: "JSON",
...
$(document).on('click', '.cargar', function(event) {
  ...
  $.ajax({
    ...
    success: function(xdatos) {
      $("#inventable>tbody").html(xdatos.data);
      $(".sel").select2();
      $(".posicion").select2();
...
function senddata()
{
  ...
  $("#inventable>tbody tr").each(function()
  {
    ...
    if (compra!="" &&parseFloat(compra) >= 0 &&venta!="" &&parseFloat(venta) >= 0 && cant != "")
    {
      datos += id_prod + "|" + compra + "|" + venta + "|" + cant + "|" + unidad + "|" + vence + "|" + id_presentacion + "|" + posicion + "#";
      i = i + 1;
    }
  });
...
function reload1()
{
  location.href = "ajuste_inventario_por_estante.php";
}
$(document).on('change', '#destino', function(event) {
