			...
			$.ajax({
				...
				success: function(datax) {
					...
					if(datax.typeinfo == "Success")
					{
						...
						$(".decimal").numeric({negative:false,decimal:false});
...
$(document).on('keyup', '#total_retencion', function(event) {
	totales();
});
...
function totales() {
	...
	var con_pago = parseInt($("#con_pago").val());
	var cargo_tarjeta = parseFloat($("#cargo_tarjeta").val());
	...
	if (tipo_impresion=="CCF") {
		...
		total_retencion1 = parseFloat($("#total_retencion").val());
		if (isNaN(total_retencion1)) {
			total_retencion1=0
		}
		...
		if (con_pago!=2) {
			...
			cargo_tarjeta= cargo_tarjeta*total_final;
			total_final = total_final + cargo_tarjeta;
			cargo_tarjeta =  cargo_tarjeta.toFixed(2);
		}
		$("#ct").html(cargo_tarjeta);
...
function senddata() {
	...
	var sumas= $('#total_gravado').text();/*total sumas sin iva + exentos*/
	var iva = $('#total_iva').text(); /*porcentaje de iva de la factura*/
	var retencion = $('#total_retencion').val();/*total retencion cuando un cliente retiene 1 o 10 %*/
	var venta_exenta =$('#total_exenta').text();/*total venta exenta*/
	var total = $('#totalfactura').val();
	...
	var cargo_tarjeta= $("#ct").text();/*total cargo tarjeta*/
	...
	var trs = $("#inventable > tr").length;
	console.log(trs);
  if((tipo_impresion=="CCF" && trs<=12) || (tipo_impresion=="COF" && trs<=12) || (tipo_impresion=="TIK"))
	{
		var verificaempleado = 'noverificar';
		var verifica = [];
		var array_json = new Array();
		...
		swal({
		title: "Advertencia",
		text: "El numero maximo de items para este tipo de documentos es 12",
		type: "error",
	});
}
}
...
$(document).on("keyup","#efectivov",function(evt){
	...
	{
		if(parseFloat($("#cambiov").val()) >=0 && parseFloat($("#efectivov").val()) >=0)
		{
			imprimev();
		}
		else {
