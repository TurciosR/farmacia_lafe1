  ...
  if ($_POST['cu']) {
    ...
    if (isset($_POST['categoria']))
    {
      ...
      while($rpos=_fetch_array($sql_ubidispo))
      {
        ...
        if ($categoria=="") {
          # code...
          $sql_ids=_query("SELECT DISTINCT stock_ubicacion.id_producto,producto.descripcion  FROM stock_ubicacion JOIN producto  ON  producto.id_producto=stock_ubicacion.id_producto WHERE stock_ubicacion.id_ubicacion=$id_ubicacion AND stock_ubicacion.id_estante=$estante AND stock_ubicacion.id_posicion=$posicion AND stock_ubicacion.cantidad>0");
        ...
        if ($cu>0) {
          ...
          $pdf->Ln(1);
          if ($des_estante=="" && $des_posicion=="") {
            // code

            $pdf->Cell(191, 5, "NO ASIGNADO", 1, 1, 'C');
          }
          else {

            $pdf->Cell(191, 5, $des_estante."  POSICION: ".$des_posicion, 1, 1, 'C');
          }
          for ($i=0;$i<$cu ;$i++)
          {
            ...
            $a=$pdf->GetX();
            $pdf->Cell(91, 5, substr(utf8_decode($sql['descripcion']),0,38), 0, 0, 'L');
            $pdf->Cell(40, 5, utf8_decode($sql['nombre']), 0, 0, 'L');
            $pdf->Cell(20, 5, $unidades, 0, 0, 'R');
            $pdf->Cell(20, 5, $existencia, 0, 0, 'R');
            $pdf->Cell(20, 5, "", 0, 1, 'L');
          }
        }
