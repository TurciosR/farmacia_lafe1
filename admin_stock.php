...
function initial()
{
  ...
  <div class="wrapper wrapper-content  animated fadeInRight">
                      ...
                      <div class="widget-header">
                        ...
                        <div class="widget-content">
                          <div class="row">
                            ...
                            <div class="col-md-3">
                              <div class="form-group">
                                <button type="button" class="btn btn-primary auto_asignar" name="button">Realizar asignación automatica</button>
                              </div>
                            </div>
                          </div>
                          ...
                          <table class="table table-bordered" id='loadtable'>
                            <thead>
                              <tr>
                                ...
                                <th class="text-success col-lg-1">Precio Marca</th>
    ...
    <script type="text/javascript">
      $(document).on('click', '.auto_asignar', function(event) {
        $(this).attr('disabled', 'disabled');
        $.ajax({
          url: 'auto_asignar.php',
          type: 'POST',
          dataType: 'json',
          data: {process: 'asignar'},
  ...
  function traerdatos()
  {
    ...
    if ($num_rows > 0)
    {
      while ($row = _fetch_array($query))
      {
        ...
        $sql_pres = _query("SELECT pp.*, p.nombre as descripcion_pr FROM presentacion_producto as pp, presentacion as p WHERE pp.presentacion=p.id_presentacion AND pp.id_producto='$id_producto' AND pp.activo=1 AND pp.id_sucursal=$_SESSION[id_sucursal] ORDER BY pp.unidad DESC");
        ...
        <tr>
          ...
          while ($rowb = _fetch_array($sql_pres))
          {
            ...
            $costo = $rowb["costo"];
            $precio = $rowb["precio"];
            $precio_marca = $rowb["precio_marca"];
