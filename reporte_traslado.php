...
class PDF extends FPDF
{
    ...
    public function Header()
    {
        ...
        $this->Cell(205,6,$this->a,0,1,'C');
        $this->SetFont('Latin','',10);
        $this->Cell(205,6,utf8_decode((Mayu(utf8_decode("Sucursal ".$this->e.": ".$this->c)))),0,1,'C');
        $this->Cell(205,6,$this->b,0,1,'C');
        $this->Cell(205,6,"REPORTE DE TRASLADO",0,1,'C');
        $this->Cell(205,6,"TRASLADO: ".str_pad($this->f,7,"0",STR_PAD_LEFT),0,1,'C');
        $this->Cell(205,6,"DESTINO: ".utf8_decode((Mayu(utf8_decode($this->g)))),0,1,'C');
        $this->Cell(205,6,$this->d." ".hora($this->w),0,1,'C');
        // Salto de línea
        $this->Ln(5);
        $set_y=$this->GetY();
        $set_x=$this->GetX();
        ...
        //$this->Cell(20, 5, 'COSTO+IVA', 1, 0, 'L');
        $this->Cell(15, 5, 'CANT', 1, 1, 'L');
...
$total=0;
$sqli=_fetch_array(_query("SELECT iva FROM sucursal LIMIT 1"));
$iva=$sqli['iva'];
$iva=round($iva/100,2);
$iva=1+$iva;
while ($row=_fetch_array($sqlp)) {
  ...
  $pdf->Cell(125, 5, utf8_decode($row['descripcion']), 1, 0, 'L');
  $pdf->Cell(40, 5, utf8_decode($row['nombre']), 1, 0, 'L');
  $pdf->Cell(15, 5, $row['unidad'], 1, 0, 'R');
  //$pdf->Cell(20, 5,"".number_format(($row['costo']*$iva),2), 1, 0, 'R');
  $pdf->Cell(15, 5,"".($row['cantidad']/$row['unidad']), 1, 1, 'R');
  //$pdf->Cell(20, 5,"".number_format((($row['cantidad']/$row['unidad'])*$row['costo']*$iva),2), 1, 1, 'R');
  $total=$total+round((($row['cantidad']/$row['unidad'])*$row['costo']*$iva),2);
}
//$pdf->Cell(185, 5,"TOTAL", 1, 0, 'C');
